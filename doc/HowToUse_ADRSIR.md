...
## pythonで学習したデータを読み込む、送信する

[ADRSIR_RaspberryPi_IR_Leaning_Controller](https://github.com/bit-trade-one/ADRSIR_RaspberryPi_IR_Leaning_Controller.git)

メーカーがgithubにコードを公開しているので、それを参考にしてください。
...
if __name__ == "__main__":
   # remote_command.trans_command(filename=filename) で指定ファイルのコードを赤外線で送信
   remote_control(ctrl_num=1)
   # remote_command.read_command(filename, memo_no)(filename, memo_no)で記憶したデータをファイルに保存
```